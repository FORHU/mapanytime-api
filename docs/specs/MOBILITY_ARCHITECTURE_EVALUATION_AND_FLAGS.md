...
- **Recommendation:**
  - **Ephemeral Session**: Store Level 2 selection in Flutter local state (`shared_preferences` / Riverpod state) with a time-to-live (e.g., auto-prompt after 45 minutes: _"Are you still on a Taxi?"_ or clear when app is terminated).
