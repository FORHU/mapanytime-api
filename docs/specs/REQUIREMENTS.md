...
*The earlier header claimed 100 requirements across a different split; it was a
stale hand-count. The figures above are a mechanical recount of the 19 tables.*
...
close. The decision and its worked example live in `FLAGS.md` under *Confirmed
business rules*; `tests/unit/pricing.engine.financial-rules.test.ts` and
